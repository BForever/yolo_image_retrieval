...
def get_test_input():
    ...
    img_ = img_[np.newaxis, :, :,
           :] / 255.0  # Add a channel at 0 (for batch) | Normalise
...
def parse_cfg(cfgfile):
    ...
    lines = [x.rstrip().lstrip() for x in
             lines]  # get rid of fringe whitespaces
    ...
    for line in lines:
        if line[0] == "[":  # This marks the start of a new block
            if len(
                    block) != 0:  # If block is not empty, implies it is storing values of previous block.
...
class Darknet(nn.Module):
    ...
    def save_features(self, filename):
        feature = self.features.detach().cpu().numpy()
        np.savez(filename, feature)
        return feature.shape[2]
        ...
        for i, module in enumerate(modules):
            ...
            if module_type == "convolutional" or module_type == "upsample":
                ...
                if module_type == "convolutional":
                    # print(i, module['filters'])
                    if i == 80:
                    # if i == 103:
                        if self.features == None:
                            self.features = x.clone().detach()
                        else:
                            self.features = torch.cat((self.features, x.clone().detach()))
